...
function getReservById (req, res) {
  ReservModel.findById(req.params.id)
    .then((response) => res.json(response))
    .catch((err) => handleError(err, res))
}

function deleteReservById (req, res) {
  ReservModel.remove({ _id: req.params.id })
    .then((response) => res.json(response))
    .catch((err) => handleError(err, res))
}

function updateReserv (req, res) {
  ReservModel.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true
  })
    .then((response) => res.json(response))
